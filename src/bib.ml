...
module Raw = struct
  module Kv = struct
    ...
    let merge kv1 ~update =
      let rec loop = function
        | [] -> []
        | (k, v) :: xs ->
            if Option.is_some (List.assoc_opt k update) then loop xs
            else (k, v) :: loop xs
      in
      update @ loop kv1
  end
  ...
  let part_to_string ?(delimiter = false) ?(variable = false) p =
    ...
    | Some _ | None -> (
        ...
        | true, `Dquote ->
            Printf.sprintf "\"%s\""
              p.text (* TODO: is any quoting of raw text needed? *))
  ...
  let fold_entries ?type' fn t acc =
    let rec loop acc = function
      ...
      | Entry { type' = type2; citation_key; tags } :: es ->
          let skip =
            Option.map (fun t -> not (t = type2)) type'
...
let names s =
  ...
  |> List.map String.trim |> List.map Name.of_string
...
let err_lookup field e = Failure (Fmt.str "No field %s in %a" field Raw.pp_kv e)
...
let of_raw : Raw.t -> t =
 fun es ->
  let convert (e : Raw.entry) : (string * entry) option =
    match e with
    ...
    | Entry e -> (
        ...
        | "article" ->
            ...
            let v =
              Article.v ~author ~year:(int_of_string year) ~journal title
            in
        ...
        | "inproceedings" ->
            ...
            let year, acc = find_exn "year" acc in
            let v =
              Inproceedings.v ~author ~year:(int_of_string year) ~booktitle:""
                title
            in
...
let to_raw : t -> Raw.t =
 fun ts ->
  let convert ~citation_key (e : entry) : Raw.entry =
    match e with
    | Article (article, e) ->
    ...
    | Other (type', tags) -> Entry { type'; citation_key; tags }
  in
...
let author = function
  ...
  | v ->
      ...
      |> Option.map Raw.text |> Option.map names |> Option.value ~default:[]

let title = function
  | Article (a, _) -> Some (Article.title a)
  | Inproceedings (a, _) -> Some (Inproceedings.title a)
  | v -> Raw.Kv.find "title" (extra v) |> Option.map Raw.text

let year = function
  | Article (a, _) -> Some (Article.year a)
  | Inproceedings (a, _) -> Some (Inproceedings.year a)
  | v ->
      Raw.Kv.find "year" (extra v) |> Option.map Raw.text |> fun v ->
      Option.bind v int_of_string_opt
...
module Textloc = struct
  ...
  let none =
    make ~filename:"-" ~first_byte:(-1) ~last_byte:0 ~first_line:(1, 0)
      ~last_line:(1, 0)
  ...
  let pp ppf l =
    ...
    | false ->
        ...
        if pos_s = 0 && pos_e = 0 then
          pf ppf "File \"%a\", %a" pp_path l.filename pp_lines l
        else
          pf ppf "File \"%a\", %a, characters %d-%d" pp_path l.filename pp_lines
            l pos_s pos_e
end
...
module Error = struct
  ...
  let () =
    Printexc.register_printer (function
      | Error (txtloc, msg) -> Some (Fmt.str "%a: %s" Textloc.pp txtloc msg)
      | _ -> None)
end
...
let err_to_here ~first_byte ~first_line d fmt =
  ...
  Error.msgf
    (Textloc.make ~first_byte ~first_line ~last_byte ~last_line
       ~filename:d.filename)
    fmt
...
let err_malformed_key ~first_byte ~first_line d =
  err_to_here ~first_byte ~first_line d "Malformed key, got a '%c'"
    (Stdlib.Char.unsafe_chr d.c)

let err_malformed_kv ~first_byte ~first_line d =
  err_to_here ~first_byte ~first_line d "Malformed set of key-values"
    (Stdlib.Char.unsafe_chr d.c)

let err_unexpected_character ~first_byte ~first_line e d =
  err_to_here ~first_byte ~first_line d
    "Unexpected character, expected '%c' but got '%c'"
...
let[@inline] token_pop d =
  let t = Buffer.contents d.token in
  token_clear d;
  t
...
(* Whitespace *)
let[@inline] is_ws u =
  if u > 0x20 then false
  else
    match Stdlib.Char.unsafe_chr u with
    | ' ' | '\t' | '\r' | '\n' -> true
    | _ -> false
...
let consume_word d =
  (* let first_byte = get_last_byte d and first_line = get_line_pos d in *)
  let rec loop d =
    if is_text d.c || is_digit d.c then (
      accept d;
      loop d)
  in
  loop d;
...
let consume_kv d =
  ...
  let value_part d =
    ...
    | 0x007B (* { *) ->
        nextc d;
        {
          Raw.text = consume_delimited_word ~delimiter:0x007D d;
          delimiter = `Curly;
          variable = None;
        }
    | 0x0022 (* DQUOTE *) ->
        nextc d;
        {
          Raw.text = consume_delimited_word ~delimiter:0x0022 d;
          delimiter = `Dquote;
          variable = None;
        }
    | _ ->
...
let consume_citation_key d =
  let rec loop d =
    if is_ascii d.c && not (is_comma d.c) then (
      accept d;
      loop d)
  in
  loop d;
...
let write_eot ~eod e =
  flush e;
  if eod then Bytes.Writer.write_eod e.writer

let write_char e c =
  if e.o_next > e.o_max then flush e;
  Stdlib.Bytes.set e.o e.o_next c;
  e.o_next <- e.o_next + 1
...
let rec write_substring e s first length =
  if length = 0 then ()
    ...
    if len = 0 then (
      flush e;
      write_substring e s first length)
    else begin
...
let write_part e { Raw.text; delimiter; variable = _ } =
...
let of_string ?(filename = "of_string") s =
  decode ~filename (Bytes.Reader.of_string s) |> of_raw
